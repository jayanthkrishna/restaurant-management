...
func GetInvoices() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		res, err := invoiceCollection.Find(ctx, bson.M{})
		defer cancel()
...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var invoiceView InvoiceViewForamt

		allOrderItems, err := ItemsByOrder(invoice.Invoice_id)

		invoiceView.Order_id = invoice.Invoice_id
		invoiceView.Payment_due_date = invoice.Paymeny_due_date
...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		invoice.Paymeny_due_date, _ = time.Parse(time.RFC3339, time.Now().AddDate(0, 0, 1).Format(time.RFC3339))
