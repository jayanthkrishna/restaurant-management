...
var validate = validator.New("", nil)
...
func UpdateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if food.Menu_id != nil {
			...
			updatedObj = append(updatedObj, bson.E{"menu_id", food.Menu_id})
		}

		food.Updated_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		updatedObj = append(updatedObj, bson.E{"updated_at", food.Updated_at})
		...
		res, err := foodCollection.UpdateOne(ctx, filter,
			bson.D{
				{"$set", updatedObj},
			},
			&opt,
